...
##  Create introduction and grab initial question as input
    ...
    print('\n\n' + """You might want to ask this character a few questions first.
You just met them, after all.""" + '\n\n' + divider)
    print("You only have time to ask " + str(questionNum) + " questions. Ask wisely!")
    for item in bastian.q_and_a.keys():
        print(str(item) + '. ' + bastian.get_question(item) + '\n')
    print(divider) 
    theQuestion = input("Choose the number key for the question you want to ask, then press Enter: ")
else:
